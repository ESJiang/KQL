...
export function injectFormatSettings() {
    ...
    if (!("editor.defaultFormatter" in existingKqlConfig)) {
        newKqlConfig["editor.defaultFormatter"] = "fangweij.kql-tools-vscode";
    }
    if (!("editor.formatOnSave" in existingKqlConfig)) {
        newKqlConfig["editor.formatOnSave"] = true;
    }
...
function formatKqlBlocks(blocks: KqlBlock[], indentLevel = 0): string {
    ...
    function pushLine(line: string, opts: { indent?: string; isFirstLine?: boolean } = {}) {
        const { indent = "", isFirstLine = false } = opts;
        if (/^(?:\s*)(\/\/|\/\*|\*\/)/.test(line)) {
            line = line.trimStart();
            if (line.startsWith("//")) {
        ...
        const startsWithNoIndent = noIndentKeywords.some(k => line.toLowerCase().startsWith(k.toLowerCase()));

        // if (result.length > 0 && startsWithNoIndent && !isFirstLine) {
        //     result.push("");
        // }
        const isNewBlock = /^(datatable|union|summarize|project)\b/i.test(line);
        if (result.length > 0 && isNewBlock) {
            result.push("");
        }
        ...
        switch (block.type) {
            case "pipe": {
                ...
                block.children?.forEach((child, idx) => {
                    ...
                    if (isPipeFunc) {
                        childNormalized = childNormalized.replace(/,\s*/g, ", ").replace(/, +$/, ",");
                    }
